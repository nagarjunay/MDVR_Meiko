 ...
 *
 */
public class TC207_VerifyVideoStatus extends TestBase  {

	LoginPage loginpage;
	VehicleReports_VideoStatus vs;
	...
	public void verifyVideoStatusOption(String loginid, String password, String runMode) throws Exception {
		if (runMode.equalsIgnoreCase("n")) {
			throw new SkipException("User marked this not to run");
		}
		log("=========>Started--->Video Status Report Module");
		loginpage = new LoginPage(driver);
		loginpage.loginToApplication(loginid, password);
		vs = new VehicleReports_VideoStatus(driver);
		vs.VideoStatus();
		vs.validation();
		vs.validation1();
		vs.validation2();
		...
		log("=========>Finished--->Video Status Report Module");
	}
